...
def temukan_dns_server():
    ...
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    s.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1) # Izinkan Broadcast
    s.settimeout(2.0)
    try:
        # Teriak ke seluruh jaringan (255.255.255.255)
        ...
        print("[!] Gagal menemukan DNS secara otomatis. Pastikan DNS Server sudah menyala.")
    s.close()
    return None
# -----------------------------
...
if pakai_dns == 'y':
    dns_ip = temukan_dns_server() # Memanggil fungsi sakti pencari DNS
    
    if dns_ip: # Jika DNS berhasil ditemukan
        ...
        try:
            ...
            print(f"[!] Gagal mendaftar ke DNS: {e}")
    else:
        print("[!] Melewati pendaftaran DNS karena server DNS tidak ditemukan.")
        ...
        try:
            ...
            if client_address in clients:
                ...
                if pesan_teks == "__KELUAR__":
                    ...
                    for addr in clients:
                        server_socket.sendto(f"[SERVER] {pengirim} meninggalkan obrolan.".encode('utf-8'), addr)
                else:
                ...
                if is_private:
                    ...
                    print(f"\n[!] ADMISSION: '{username_baru}' (ID {next_pending_id}) dari IP {client_address[0]}:{client_address[1]} ingin bergabung!")
                    print("Admin> ", end="", flush=True)
                    next_pending_id += 1
                else:
