...
def jalankan_dns():
    ...
    while dns_berjalan:
        try:
            ...
            # --- FITUR BARU: MENGHAPUS SERVER HANTU ---
            elif command == 'DEREGISTER':
                name = parts[1]
                if name in registry:
                    del registry[name]
                    print(f"[-] Server '{name}' telah mematikan diri dan dihapus dari DNS.")
            # ------------------------------------------
...
while True:
    try:
        ...
        if perintah == 'stop':
            ...
            if not registry: print("Belum ada server yang terdaftar.")
            else:
                for nama, (ip, port) in registry.items(): print(f" - [{nama}] beroperasi di {ip}:{port}")
            print("------------------------------\n")
        elif perintah == 'reset':
            registry.clear()
